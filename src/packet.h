...
#define PKT_LOG(p)  LOG_C("pos:%d len=%d err=%d buf=%s\n", (p)->pos, (p)->len, (p)->error, (p)->buf);
...
inline void packet_end(packet *p) { p->buf[p->len] = '\0'; }
inline bool packet_empty(packet *p) { return p->buf[p->pos] == '\0'; }
inline uint8_t *packet_ptr(packet *p) { return &p->buf[p->pos]; }
...
int packet_take_int_base(packet *p, uint8_t base);
int packet_take_int(packet *p);
...
int packet_take_arg(packet *p, int def);
