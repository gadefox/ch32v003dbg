...
inline void print_num(uint8_t spaces, const char *name, uint32_t value) {
  print_b(spaces, name);
  printf(": %d\n", value);
}
...
inline void print_float(uint8_t spaces, const char *name, float value) {
  print_b(spaces, name);
  printf(": %f\n", value);
}
...
inline void print_hex(uint8_t spaces, const char *name, uint32_t value) {
  print_b(spaces, name);
  printf(": %08X\n", value);
}
...
inline void byte_to_hex(uint8_t b, uint8_t *hex) {
  *hex++ = to_hex(b >> 4);
  *hex = to_hex(b & 0xF);
}
...
inline void print_str(uint8_t spaces, const char *name, const char *text) {
  print_b(spaces, name);
  printf(": %s\n", text);
}
...
inline void print_status(bool status) {
  if (status)
    print_g(1, "ok\n");
  else
    print_r(1, "failed\n");
...
void cled_set_color(uint8_t idx) {
  CHECK(idx < count_of(cled_colors))
