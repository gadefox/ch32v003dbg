...
uint32_t packet_take_hex_digits(packet *p, uint8_t digits) {
  ...
  while (digits > 0) {
    if (pos >= p->len || !from_hex_check(p->buf[pos], &digit)) {
      p->error = true;
      return 0;
    }
...
bool packet_put_hex_u16(packet *p, uint16_t w) {
  ...
  return packet_put_hex_u8(p, LOBYTE(w)) &&
         packet_put_hex_u8(p, HIBYTE(w));
}
...
bool packet_put_hex_u32(packet *p, uint32_t x) {
  // must be little-endian
  return packet_put_hex_u16(p, LOWORD(x)) &&
         packet_put_hex_u16(p, HIWORD(x));
}
